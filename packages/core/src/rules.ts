...
function doPlay(board: BoardType, play: PlayType) {
  switch (play.kind) {
    case 'jump':
      doJump(board, play);
      break;
...
export function makeRules(board: BoardType): Rules {
  return {
    ...
    doJump: (jump: JumpType) => doJump(board, jump),
    doMove: (move: MoveType) => doMove(board, move),
    doPlay: (play: PlayType) => doPlay(board, play),
