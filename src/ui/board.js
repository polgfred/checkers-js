...
export default class Board extends Component {
  ...
  renderRows() {
    ...
    for (let y = 7; y >= 0; --y) {
      elems.push(<tr key={y}>{this.renderSquares(y)}</tr>);
    }
  ...
  renderSquares(y) {
    let { board, canDrag, canDrop, endDrag } = this.props,
      elems = [];
...
@DropTarget(
  'piece',
  {
    canDrop: (props, monitor) =>
      props.canDrop({ x: props.x, y: props.y }, monitor.getItem()),
...
@DragSource(
  'piece',
  {
    ...
    endDrag: (props, monitor) =>
      props.endDrag(monitor.getItem(), monitor.getDropResult()),
  },
  (connect, monitor) => ({
